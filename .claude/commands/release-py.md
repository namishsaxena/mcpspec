...
Find the last Python release tag and collect commits since then:

```bash
git tag -l "vPY-*" --sort=-v:refname | head -1
```

If no tag exists, this is the first release — use the version in `packages/python/pyproject.toml`.
...
`**Full Changelog**: https://github.com/namishsaxena/mcpspec/compare/vPY-{old}...vPY-{new}`
...
Ask the user: "Ready to push? This will trigger the PyPI publish workflow."
